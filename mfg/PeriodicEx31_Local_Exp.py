...
"""
We solve the following MFG with an explicit solution

-v_t - v_{xx} + v_x^2/2 + a*(sin(2 * pi * x))*v_x + V(t, x) = m,
m_t - m_{xx} - (m(v_x + a * sin(2 * pi * x)))_x = 0, 

where V(t, x) = 1 + self.alpha ** 2 * jnp.sin(2 * jnp.pi * x) ** 2 / 2 - 2 * jnp.pi * self.alpha * jnp.cos(2 * jnp.pi * x) + t.

The explicit solution is given by

m(t, x) = 1,
v(t, x) = t ** 2 / 2 + self.alpha * jnp.cos(2 * jnp.pi * x) / (2 * jnp.pi).
"""

class PeriodicEx31(object):
    ...
    def m0(self, x):
        return 1    # we impose the uniform distribution on [-1, 1]

    def uT(self, x):
        return self.T ** 2 / 2 + self.alpha * jnp.cos(2 * jnp.pi * x) / (2 * jnp.pi)
    ...
    def V(self, t, x):
        return 1 + self.alpha ** 2 * jnp.sin(2 * jnp.pi * x) ** 2 / 2 - 2 * jnp.pi * self.alpha * jnp.cos(2 * jnp.pi * x) + t

    def b(self, x, m):
        return self.alpha * jnp.sin(2 * jnp.pi * x)
    ...
    def hjb(self, t, Uk1, Uk, Mk1):
        ...
        Vvals = vmap(lambda x: self.V(t, x))(self.X)

        return -Dt_U - self.nu * Delta_U + Hamiltonian + Vvals - Mk1
    ...
    def fp(self, Uk, Mk1, Mk):
        ...
        # adj_jacobi = jnp.transpose(jacfwd(self.hamilton)(Uk, Mk1))
        # adj = jnp.dot(adj_jacobi, Mk1)
        ...
        # Delta_U = - (2 * Uk - UR - UL) / self.h ** 2
        #
        # bx = vmap(lambda x: grad(self.b)(x, Mk1))(self.X)
        #
        # _r = - Delta_U - bx
    ...
    def hjb_sys(self, U, M):
        ...
        #     hjbi = self.hjb(ts[i], U[i+1, :], U[i, :], M[i+1, :])
        #     #print(jnp.linalg.norm(hjbi - hjbs[i]))
        return hjbs
    ...
    def fp_sys(self, U, M):
        ...
        # for i in range(self.Nt):
        #     fpi = self.fp(U[i, :], M[i+1, :], M[i, :])
        #     #print(jnp.linalg.norm(fpi - fps[i]))
        return fps
    ...
    def mfg(self, Z):
        ...
        Z:      vector
            should be viewed as a 2 Nt x N matrix, containing unknowns of U and M
        """
        U, M = self.prolong(Z[:self.Nt * self.N], Z[self.Nt * self.N:])

        # Next, prepare the HJB equations and the FP equations
        hjbs = self.hjb_sys(U, M)
        fps = self.fp_sys(U, M)

        return jnp.concatenate((hjbs.flatten(), fps.flatten()))

    def v(self, t, x):
        return t ** 2 / 2 + self.alpha * jnp.cos(2 * jnp.pi * x) / (2 * jnp.pi)

    def solve_mfg(self, lr, tol=10**(-6), epoch=100):

        Z = jnp.zeros((2 * self.Nt, self.N)).flatten()

        # TT = jnp.linspace(0, self.T, self.Nt + 1)
        # XX = jnp.linspace(self.xl, self.xr, self.N, endpoint=False)
        # TT, XX = jnp.meshgrid(TT, XX)
        #
        # points = jnp.concatenate((TT.T.reshape((-1, 1)), XX.T.reshape((-1, 1))), axis=1)
        #
        # vs = jnp.reshape(vmap(self.v)(points[:, 0], points[:, 1]), TT.T.shape)
        #
        # Z = jnp.ones((2 * self.Nt, self.N))
        # Z = Z.at[:self.Nt, :].set(vs[:-1, :])
        # Z = Z.flatten()
        #
        # res = self.mfg(Z)
        ...
        while error > tol and iter_num < epoch:
            b = self.mfg(Z)
            jacobi = jacfwd(self.mfg)(Z)
            dz = jnp.linalg.solve(jacobi, b)
            Z = Z - lr * dz

            error = jnp.dot(dz, dz)
            print('Error: {}, Epoch: {}'.format(error, iter_num))
